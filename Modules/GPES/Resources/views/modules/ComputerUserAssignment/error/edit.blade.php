        ...
        <div class="card-body">
            ...
            <form action="{{ route('gpes.cuentadante.assignmentsUserComputer.searchUser') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('document_number') is-invalid @enderror" id="document_number" name="document_number" value="{{ old('document_number', $person->document_number) }}" placeholder="Ingrese Cédula" required>
            ...
            @if(isset($person) && $person)
                <h2 class="mt-4">Usuario seleccionado</h2>
                <table class="table">
                    <thead>
                        <tr>
                            <th>Documento</th>
                ...
                <form action="{{ route('gpes.cuentadante.assignmentsUserComputer.update', $assignment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="user_id" value="{{ old('user_id', $assignment->user_id) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                            <option value="diario" {{ old('type', $assignment->type) == 'diario' ? 'selected' : '' }}>Diario (1 día)</option>
                            <option value="formacion" {{ old('type', $assignment->type) == 'formacion' ? 'selected' : '' }}>Formación (personalizado)</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control @error('assigned_at') is-invalid @enderror" id="assigned_at" name="assigned_at" value="{{ old('assigned_at', $assignment->assigned_at->format('Y-m-d\TH:i')) }}" required>
                        @error('assigned_at')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control @error('returned_at') is-invalid @enderror" id="returned_at" name="returned_at" value="{{ old('returned_at', $assignment->returned_at ? $assignment->returned_at->format('Y-m-d\TH:i') : '') }}">
                        @error('returned_at')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('observation') is-invalid @enderror" id="observation" name="observation" rows="4">{{ old('observation', $assignment->observation) }}</textarea>
                        @error('observation')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="returned" class="form-label">Devuelto</label>
                        <input type="checkbox" id="returned" name="returned" value="1" {{ old('returned', $assignment->returned) ? 'checked' : '' }}>
                    </div>
