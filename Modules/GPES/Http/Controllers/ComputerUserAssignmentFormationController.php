...
class ComputerUserAssignmentFormationController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $assignments = ComputerUserAssignment::where("type", "formacion")
            ->with(["computer", "user"])
            ->orderBy("created_at", "desc")
            ->get();
        return view('gpes::modules.ComputerUserAssignment.formation.index', compact("assignments"));
    }
    ...
    public function create()
    {
         ...
         $computers = Computer::where("status_assignment_formation", "disponible")->get();
       return view('gpes::modules.ComputerUserAssignment.formation.create', compact("computers"));
    }
    ...
    public function store(Request $request)
    {
       ...
       if(!$peopleUser){
        ...
        $endDate = Carbon::parse($request->returned_at)->format("Y-m-d H:i:s");
       ...
       $assignment = ComputerUserAssignment::create([
            ...
            'returned_at' =>  $endDate, // se alamcena la fecha actual pero a la ultima hora del dia
        ...
        ->update([
            'status_assignment_formation' => 'asignado',
        ]);
     

        return redirect()->route('gpes.cuentadante.assignmentsUserComputer.formation.index')->with('success', 'Asignación creada exitosamente.');
   ...
   public function return($id)
    {
        ...
        if($date->gt($assignment->returned_at)){
            ...
            return redirect()->route('gpes.cuentadante.assignmentsUserComputer.formation.index')->with('success', 'Computador devuelto exitosamente tarde.');
        }
        ...
        // si la fecha actual es menor a la fecha de entrega, se marca como devuelto
            ...
            return redirect()->route('gpes.cuentadante.assignmentsUserComputer.formation.index')->with('success', 'Computador devuelto exitosamente a tempo.');
        }
     ...
     public function edit($id)
    {
        ...
        $computers = Computer::where('status_assignment_formation', 'disponible')
            ->orWhere('id', $assignment->computer_id)
            ->get();

        return view('gpes::modules.ComputerUserAssignment.formation.edit', compact('assignment', 'computers'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $computer->update(['status_assignment_formation' => 'asignado']);

            DB::commit();

            return redirect()->route('gpes.cuentadante.assignmentsUserComputer.formation.index')
